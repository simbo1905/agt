...
| **Standalone snapshot store** | Git | A separate bare repository used by `agt snapshot ...` |
| **Standalone snapshot** | Git+Disk | A tagged commit in the standalone snapshot store plus a manifest blob of filesystem metadata |
...
- **Standalone Evidence Capture**: `agt snapshot ...` stores arbitrary tree snapshots in a separate bare repository so generated output can be checked and restored without touching normal repo history.
...
The standalone snapshot feature intentionally reuses Git object storage in a second, isolated repository. It is additive: it does not reuse the main repository index, does not modify user branches, and does not interfere with autocommit or session restore flows.
...
This section describes session shadow restore. It is distinct from `agt snapshot restore`, which operates on the standalone snapshot store.
...
## 7. Standalone Snapshot Store

The standalone snapshot feature is a separate additive subsystem.

- Default store: `.agt-snapshots/` in the current working directory
- Override: `--store <path>` or `AGT_SNAPSHOT_STORE`
- Warning: AGT warns if the chosen store location is not ignored by Git

Each standalone snapshot save does the following:

1. Scans the target tree directly from disk, including files ignored by Git.
2. Skips the normal `.git/` directory and the snapshot store itself.
3. Writes file content blobs into the standalone snapshot store.
4. Writes a manifest blob with extra filesystem metadata, such as object id, file id, parent file id, timestamps, ownership, mode, and platform flags where available.
5. Creates a tree and commit in the standalone snapshot store.
6. Creates an annotated tag named from the Unix epoch of the snapshot.

`agt snapshot status` compares the current tree against the latest standalone snapshot. `-q` prints a one-line clean or changed result. `-qq` is for scripts and exits non-zero on the first detected mismatch.

`agt snapshot restore` has two modes:

- Full restore: requires the latest snapshot to match the current filesystem first, so the user has a fresh backup.
- Targeted restore: accepts one or more paths and can recover deleted files or folders without first taking a new backup snapshot.

For safety, targeted restore refuses to clobber by default and prompts before overwriting existing files.
...
### `agt snapshot save/check/status/restore`

```mermaid
sequenceDiagram
    autonumber
    participant agt_cli as "agt CLI"
    participant target_tree as "Target tree"
    participant snapshot_store as "Standalone snapshot store"

    agt_cli->>target_tree: Scan files directly from disk, including gitignored output
    agt_cli->>snapshot_store: Write blobs for file contents
    agt_cli->>snapshot_store: Write manifest blob with filesystem metadata
    agt_cli->>snapshot_store: Create tree, commit, and annotated tag
    agt_cli->>snapshot_store: Compare latest or requested snapshots for status/check
    agt_cli->>target_tree: Restore all or selected paths when requested
```
